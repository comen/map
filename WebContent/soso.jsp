...
<script>
	var init = function() {
		...
		var initPath = [ new qq.maps.LatLng(31.33795, 121.40150),
		...
		qq.maps.event.addListener(polygon, 'rightclick', function(event) {
			...
			var file = fso.OpenTextFile("C:/path.txt", 2, true);
			var length = latLngs.getLength();
			for (var i = 0; i < length; i++) {
				var latLng = latLngs.getAt(i);
				var lat = latLng.getLat().toFixed(5), lng = latLng.getLng()
						.toFixed(5);
		...
		var infoWin = new qq.maps.InfoWindow({
			content : '(' + center.getLat() + "," + center.getLng() + ")",
			position : center,
			map : map
		});
		...
		qq.maps.event.addListener(polygon, 'mouseout', function(event) {
			infoWin.close('北区局');
		});
		qq.maps.event.addListener(map, 'zoom_changed', function(event) {
